...
import { PayoutError, requestDonationRecipientCorrection } from "@/lib/payouts/service.server";
...
export async function POST(request: Request, context: { params: Promise<{ allocationId: string }> }) {
  ...
  try {
    ...
    const result = await requestDonationRecipientCorrection(authorization.discord_user_id, {
    ...
    return NextResponse.json({ error: error instanceof Error ? error.message : "The charity change could not be started." }, { status, headers: { "Cache-Control": "private, no-store" } });
  }
}
