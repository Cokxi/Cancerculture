...
import { ProfileSubmissionList } from "../ProfileHistoryLists";

const PATH = "/my-profile/submissions";
...
export default async function MySubmissionsPage() {
  ...
  if (sessionState.status === "dependency_unavailable") {
    return (
      <main className="mx-auto max-w-5xl px-4 py-10 text-white">
        ...
        <p role="status" className="rounded-2xl border border-white/10 bg-black/70 p-8 text-center">Submissions are temporarily unavailable.</p>
      </main>
    );
  }
  ...
  return (
    <main className="mx-auto max-w-5xl space-y-8 px-4 py-10 text-white">
      <BackButton href="/" label="Home" />
      <h1 className="font-['Permanent_Marker'] text-4xl text-[var(--orange-dark)]">My Submissions</h1>
      <ProfileSubmissionList submissions={profile.submissions} />
    </main>
  );
}
